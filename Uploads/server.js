...
app.get('/api/portfolio/binance', async (req, res) => {
  try {
    ...
    // Calculate total value (simplified - in production, fetch current prices)
    const totalValue = balances.reduce((sum, balance) => {
      const amount = parseFloat(balance.total);
      // Simplified price calculation - replace with real price API
      const estimatedPrice = balance.asset === 'BTC' ? 58000 : 
                           balance.asset === 'ETH' ? 2650 : 
                           balance.asset === 'USDT' ? 1 : 100;
